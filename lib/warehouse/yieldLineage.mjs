 ...
 * @property {string|null} observationId   upstream observation id
 * @property {boolean} observationVerified  the id was VERIFIED to exist via a trusted lookup/join (a random non-null id must be false)
...
function inputReasons(side, e, opts) {
  ...
  if (e.observationVerified !== true) r.push(`${side}: observation not verified via lookup`);
...
export function qualifyYield(ev, opts) {
  ...
  const disposition =
    has("observation id") || has("not verified via lookup") || has("provenance") || has("source contract") || has("source id") || has("ASGS")
      ? "lineage_unverified"
