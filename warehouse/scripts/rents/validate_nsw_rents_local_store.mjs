#!/usr/bin/env node
/**
 * NSW rents local store validation — pilot (Sprint 6, Part C).
 *
 * Read-only validation of warehouse/data/local/nsw_rents.duckdb against the
 * local ASGS backbone store. No Supabase connection, no secrets.
 *
 * Outputs (committed):
 *   warehouse/reports/nsw_rental_bonds_local_store_report.json
 *   warehouse/reports/nsw_rental_bonds_local_store_report.md
 */
...
const OUT_JSON = rel("warehouse", "reports", "nsw_rental_bonds_local_store_report.json");
const OUT_MD = rel("warehouse", "reports", "nsw_rental_bonds_local_store_report.md");
...
for (const [p, hint] of [
  [DB_PATH, "run build_nsw_rents_local_store.mjs"],
  [ASGS_DB, "run build_asgs_local_store.mjs (Sprint 2)"],
  [INVENTORY, "run build_nsw_rents_local_store.mjs"],
]) {
...
const passed =
  ...
  checks.non_positive_rent === 0 && checks.poa_unmatched_to_asgs === 0 &&
  byQuarter.length > 0;
...
const md = `# NSW Rental Bonds Local Store Report (Sprint 6 Pilot)
...
console.log("  warehouse/reports/nsw_rental_bonds_local_store_report.json");
console.log("  warehouse/reports/nsw_rental_bonds_local_store_report.md");
